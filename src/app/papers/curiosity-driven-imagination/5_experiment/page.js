...
export default function ExperimentPage() {
  return (
    <article className="prose max-w-none">
      <SectionHeader title="Experiment" subtitle="実験" sectionNumber="V" />
      ...
      <SubsectionContainer title="実験環境" id="experiment-environment">
        <p>
          我々はRoboSuite[41]の「Pick and Place
          Can」タスクで本手法を評価しました。このタスクは単純に見えますが、強化学習エージェントにとって挑戦的です[42]。ロボットアームは缶をピックアップしてビン（収納箱）に配置する必要があります。
        </p>
        ...
        <p>
          図3は実験環境を示しています。左側は元々の「Pick and
          Place」タスクで、エージェントが缶をピックアップしてドロップオフビンに配置する必要があります。右側は「Locked
          Door」という新規性シナリオで、ドアがドロップオフビンへのアクセスをブロックしています。このシナリオでは、エージェントは最初に近接センサー（青いボール）を介してドアのロックを解除し、その後ドアを押して開ける必要があります。赤いボールはライトスイッチの位置を示しています。
        </p>
        ...
        <AITips>
          <p>
            実験環境の選択は非常に戦略的です。RoboSuiteの「Pick and Place
            Can」タスクは以下の理由で優れた検証環境となります：
          </p>
          <ul>
            <li>
              <strong>シンプルさと挑戦性のバランス</strong>
              ：タスク自体は「物体を掴んで別の場所に置く」という概念的にシンプルなものですが、ロボット制御の観点からは関節の協調動作や物体認識など複雑な要素が含まれています。
            </li>
            <li>
              <strong>新規性の導入しやすさ</strong>
              ：基本タスクに様々な「新規性」（ドアやライトスイッチなど）を追加することで、エージェントの適応能力を段階的にテストできます。
            </li>
            <li>
              <strong>現実世界との関連性</strong>
              ：物体操作は実世界のロボットにとって基本的かつ重要なタスクです。このタスクでの適応能力は、実用的なロボットシステムの開発に直接関連します。
            </li>
          </ul>
          <p>
            「Locked
            Door」のような新規性シナリオは、特に興味深いテストケースです。このシナリオでは、エージェントは単に「何かをどこかに移動させる」以上のことを学ぶ必要があります。エージェントは「ドアをロック解除してから開ける」という新しい概念を学び、それを目標達成のための前提条件として理解する必要があります。これは人間の赤ちゃんが因果関係や条件付きの行動連鎖を学ぶ過程に似ています。
          </p>
        </AITips>
      </SubsectionContainer>

      <SubsectionContainer title="比較手法" id="baseline-methods">
        <p>Bi-Modelの性能を評価するために、著者らは以下の代表的な手法と比較実験を行っています：</p>
        ...
        <ul>
          ...
          <li>
            <strong>LEAP</strong>：シンボリック計画と強化学習を組み合わせた手法（Nasiriany et al.,
            2019）。
          </li>
          <li>
          ...
          phases={[
            {
              title: 'フェーズ1: 事前学習',
              description:
                '各手法で環境を探索し、基本的なスキル（オペレータ）を学習します。この段階では特定の目標は与えられず、環境の理解に焦点を当てます。',
              duration: '5000エピソード',
            ...
            },
            {
              title: 'フェーズ2: 適応学習',
              description:
                '特定の目標が与えられ、事前学習で獲得したスキルを用いて目標達成を目指します。新しい目標に対する適応性を評価します。',
              duration: '1000エピソード',
            ...
            },
            {
              title: 'フェーズ3: 転移学習',
              description:
                '環境が変化（新しいオブジェクト追加、障害物配置変更など）した際の適応性を評価します。事前学習で獲得したスキルが新環境でも有効かを検証します。',
              duration: '1000エピソード',
      ...
      <SubsectionContainer title="実装詳細" id="implementation-details">
        <p>Bi-Modelの実装詳細は以下の通りです：</p>

        <h4 className="text-lg font-semibold mb-2 mt-4">1. ニューラルネットワーク構造</h4>
        <ul>
          <li>
            <strong>方策ネットワーク</strong>
            ：3層のMLP（多層パーセプトロン）、各層256ユニット、ReLU活性化関数
          </li>
          <li>
            <strong>ICM予測モデル</strong>
            ：Encoder-Decoderアーキテクチャ、畳み込み層とTranspose畳み込み層
          </li>
        </ul>
      ...
      <AITips>
        ...
        <ul>
          ...
          <li>
            <strong>比較手法の選定</strong>
            ：好奇心のみ（ICM）、計画のみ（LEAP）、その両方を部分的に持つもの（Plan2Explore）と
            体系的に比較することで、提案手法の利点を明確に示しています。
          </li>
          <li>
            <strong>多様な評価指標</strong>
            ：単に「タスクを解けたか」だけでなく、環境カバレッジやオペレータ発見数など、
            手法の内部メカニズムの働きを示す指標も含めています。これにより、「なぜ」提案手法が優れているのかの理解が深まります。
          </li>
        </ul>
        <p>
          実装詳細においても、再現性を確保するための十分な情報が提供されています。特に報酬スケーリング係数（λ
          <sub>ICM</sub>とλ<sub>PRM</sub>）は
          好奇心と計画のバランスを決める重要なハイパーパラメータであり、その設定値が明記されていることは価値があります。
          λ<sub>PRM</sub>の方がλ<sub>ICM</sub>より大きい（0.5 &gt;
          0.1）ことから、著者らは計画に基づく報酬をより重視していることがわかります。
        </p>
      </AITips>
    </article>
  );
