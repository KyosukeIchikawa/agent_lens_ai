...
export default function ResultsPage() {
  return (
    <article className="prose max-w-none">
      <SectionHeader title="Results" subtitle="結果" sectionNumber="VI" />
      ...
      <SubsectionContainer title="全体的な結果" id="overall-results">
        <p>
          我々はRoboSuiteのGripperタスク（図3）を使用して実験を行いました。このタスクでは小さな缶を把握して指定された場所に配置する必要があります。我々は5種類の新規性シナリオを導入しました：1)
          缶の初期位置の変化（Object Displacement）、2) 障害物の配置（Obstacle）、3)
          穴の出現（Hole）、4) 部屋の照明が消える（Light Off）、5)
          収納ビンへのドアがロックされる（Locked）。これらの新規性は、Shiftタイプ（1-3）とDisruptionタイプ（4-5）に分類されます。
        </p>
        ...
        <p>
          表1は、これらの新規性シナリオにおける各手法の性能を示しています。指標としては、成功率（SR）と新規性への適応までに必要なステップ数（
          <MathEquation latex="T_{adapt}" />
          ）を用いています。
          <MathEquation latex="T_{adapt}" />
          は手法が80%の成功率に達するまでのステップ数として測定され、低いほど良いことを示します。「-」は、最大ステップ数（500,000）でも80%の成功率に達しなかったことを意味します。
        </p>
        ...
        <table className="min-w-full border-collapse border border-gray-300 my-4">
          ...
          <tbody>
            <tr>
              <td className="border border-gray-300 p-2 font-bold" rowSpan="2">
                Bi-Model
              </td>
        ...
        <p>
          結果として、Bi-Modelは全てのシナリオにおいて、最高の成功率と最速の適応時間を示しました。特に、より複雑なLight
          OffやLockedの新規性シナリオでは、その優位性が顕著でした。
        </p>
      </SubsectionContainer>

      <SubsectionContainer title="新規性シナリオ別の結果" id="results-by-novelty-scenario">
        <p>以下に、各新規性シナリオにおける結果の詳細な分析を示します。</p>

        <h4 className="text-lg font-semibold mb-2">
          Shift型新規性（Object Displacement、Obstacle、Hole）
        </h4>
        <p>
        ...
        <ul>
          <li>
            Object
            Displacementシナリオ：Bi-Modelは25,000ステップで98%の成功率に達し、次に良いHyGOAL（38,000ステップで95%）を大きく上回りました。
          </li>
          <li>
            Obstacleシナリオ：Bi-Modelは42,000ステップで95%の成功率を達成し、HyGOAL（62,000ステップで92%）より効率的に適応しました。
          </li>
          <li>
            Holeシナリオ：Bi-Modelは67,000ステップで92%の成功率を示し、HyGOAL（94,000ステップで90%）より27%速く適応しました。
          </li>
        </ul>
        ...
        <ul>
          <li>
            Light
            Offシナリオ：Bi-Modelは118,000ステップで94%の成功率に達し、HyGOAL（198,000ステップで88%）と比較して40%以上速く適応しました。
          </li>
          <li>
            Lockedシナリオ：Bi-Modelは182,000ステップで90%の成功率を達成し、HyGOAL（287,000ステップで85%）より36%以上速く適応しました。このシナリオでは、純粋な強化学習アプローチ（SACとHER）は最大ステップ数でも80%の成功率に達することができませんでした。
          </li>
        </ul>
      </SubsectionContainer>
      ...
      <SubsectionContainer title="アブレーション実験の結果" id="ablation-study-results">
        ...
        <p>
          ICMを除去すると、エージェントの探索能力が大幅に低下しました。これは特に、Light
          Offシナリオで顕著でした。このシナリオでは、検出が計画に役立つオペレータを抽象化するほど密でない場合、ICMなしでは適応が困難でした。
        </p>
      ...
      <SubsectionContainer title="学習曲線" id="learning-curves">
        <p>
          {getFigNum(paperData, 'learning-curves')}は、各手法のLight
          OffとLockedシナリオにおける学習曲線を示しています。
        </p>

        <PaperFigure paper={paperData} figureId="learning-curves" width={700} height={350} />

        <p>学習曲線から、以下の重要な点が観察されました：</p>

        <ul>
          <li>
            Bi-Modelは両方のシナリオで最も急速な学習を示し、最終的に最高の成功率に達しました。
          </li>
          <li>
            特にLockedシナリオでは、Bi-Modelが約150,000ステップで急激に性能を向上させる様子が見られます。これは、エージェントが新しいオペレータを発見し、それを効果的に活用できるようになった瞬間を示しています。
          </li>
          <li>
            SACとHERなどの純粋な強化学習アプローチは、特に複雑なシナリオで学習が遅く、最終的な性能も低い傾向がありました。
          </li>
        </ul>
      </SubsectionContainer>
      ...
      <SubsectionContainer title="サンプル効率性" id="sample-efficiency">
        ...
        <ol>
          <li>
            <strong>ICMによる効率的な探索</strong>
            ：内発的好奇心モジュールは、エージェントを未知の状態へと導き、限られたサンプルから多くの情報を得ることができます。
          </li>
          <li>
            <strong>PRMによる報酬の高密度化</strong>
            ：計画報酬マシンは、スパース報酬環境において報酬信号を高密度化し、より効率的な学習を可能にします。
          </li>
          <li>
            <strong>シンボリック・サブゴール設定</strong>
            ：LTL式を通じて設定されるサブゴールにより、エージェントは複雑なタスクを管理可能な部分に分割し、各部分に対して効率的に学習できます。
          </li>
        </ol>

        <p>
          特に複雑なシナリオ（Light
          OffやLocked）では、Bi-Modelは比較手法の40%以下のサンプルで同等以上の性能を達成しました。この効率性は、現実世界のロボット応用において特に重要です。
        </p>
      </SubsectionContainer>

      <AITips>
        <p>実験結果から見えてくる重要な洞察をより身近な例で考えてみましょう。</p>
        <p>
          Bi-Modelが特に複雑なシナリオ（Light
          OffやLocked）で優れていた理由は、人間の問題解決アプローチに似ています。例えば、暗闇のアパートで電源ヒューズが落ちた場合を考えてみましょう：
        </p>
        <ul>
          <li>
            <strong>純粋な強化学習アプローチ（SACやHER）</strong>
            は、ランダムな行動と結果から学ぶような「手探り」方式です。これは暗闇の中で何度も壁にぶつかりながら、試行錯誤でヒューズボックスを探すようなものです。
          </li>
          <li>
            <strong>PRMのみのアプローチ</strong>
            は、「ヒューズボックスは玄関の近くにあるはずだ」という知識に基づいた計画を立てるようなものですが、予想外の障害物（床に置かれた荷物など）に対応できません。
          </li>
          <li>
            <strong>ICMのみのアプローチ</strong>
            は、「この壁に沿って探索してみよう」という好奇心主導の探索ですが、目標（ヒューズボックス）を見つける効率的な方法がありません。
          </li>
          <li>
            <strong>Bi-Model</strong>
            は、「ヒューズボックスは玄関の近くにあるはずだ」という計画と、「暗いから壁に沿って探索しよう」という好奇心駆動型の探索を組み合わせます。壁に沿って進み、玄関周辺で集中的に探索することで、効率的にヒューズボックスを見つけられます。
          </li>
        </ul>
        <p>
