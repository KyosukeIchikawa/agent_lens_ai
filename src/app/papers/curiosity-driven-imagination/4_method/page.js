...
export default function MethodPage() {
  return (
    <article className="prose max-w-none">
      <SectionHeader title="Method" subtitle="手法" sectionNumber="IV" />
      ...
      <SubsectionContainer title="Bi-Modelアーキテクチャ" id="bi-model-architecture">
        <p>
          Bi-Modelはその名の通り、2つの主要なモデルから構成されています： 計画報酬マシン（Planning
          Reward Machine, PRM）と 内発的好奇心モジュール（Intrinsic Curiosity Module, ICM）です。
        </p>

        <PaperFigure paper={paperData} figureId="bi-model-architecture" width={700} height={400} />

        <p>
          図1に示すように、PRMはシンボリックな遷移（あるいは一連の遷移）から生成され、これらの遷移はオペレータ抽象化関数によって直接抽象化されます。PRMはエキスパートがLTL式を使用して記述することもでき、一般的に異なる型、特に数値型が関与する場合、明示的に表現するのが難しい式を記述することができます[39,
          23, 8, 38, 37]。
        </p>

        <p>
          ICMは好奇心駆動の報酬を生成するために使用され、エージェントを予測が難しい状態へと導きます。エージェントが新しいオペレータを学習するためには、珍しい状態と例外的な遷移を経験することが重要です。ICMは、この探索を推進するための適切な報酬を提供します[32,
          33]。
        </p>
      ...
      <SubsectionContainer title="オペレータの抽象化" id="operator-abstraction">
        ...
        <PaperFigure paper={paperData} figureId="operator-abstraction" width={650} height={350} />

        <p>オペレータの抽象化プロセスは、以下のステップで行われます：</p>

        <h4 className="text-lg font-semibold mb-2 mt-4">1. 述語抽象化関数</h4>
        <p>
          状態を抽象化するために、述語抽象化関数{' '}
          <MathEquation latex="f_{\text{abs}}: S \rightarrow 2^P" /> が定義されます。 ここで{' '}
          <MathEquation latex="S" /> は状態空間、
          <MathEquation latex="P" /> は述語の集合、
        ...
        <p>
          オペレータ <MathEquation latex="o" /> は、前提条件{' '}
          <MathEquation latex="pre(o) \subseteq P" />、 追加リスト{' '}
          <MathEquation latex="add(o) \subseteq P" />
          、削除リスト <MathEquation latex="del(o) \subseteq P" /> の
          組として定義されます。オペレータが状態 <MathEquation latex="s" />{' '}
          に適用可能であるためには、 前提条件がその状態で満たされている必要があります（
          <MathEquation latex="pre(o) \subseteq f_{\text{abs}}(s)" />
          ）。
        </p>

        <p>
          オペレータの適用後の状態 <MathEquation latex="s'" />{' '}
          の述語集合は以下のように計算されます：
        </p>
        ...
        <p>
          ...
          それに対応する新しいオペレータを発見または更新します。具体的には、状態{' '}
          <MathEquation latex="s" /> から 行動 <MathEquation latex="a" /> によって状態{' '}
          <MathEquation latex="s'" /> に遷移したとき、 以下のようにオペレータが更新されます：
        </p>

        <PaperFigure paper={paperData} figureId="operator-discovery" width={650} height={300} />
        ...
        <ul>
          ...
          <li>
            <strong>追加リスト</strong>：
            <MathEquation latex="add(o) = f_{\text{abs}}(s') \setminus f_{\text{abs}}(s)" />
            （<MathEquation latex="s'" /> で新たに真となる述語）
          </li>
          <li>
            <strong>削除リスト</strong>：
            <MathEquation latex="del(o) = f_{\text{abs}}(s) \setminus f_{\text{abs}}(s')" />
      ...
      <SubsectionContainer title="アクション誘導計画" id="action-guided-planning">
        ...
        <p>
          アルゴリズム1では、まず現在の状態と目標状態を述語抽象化関数{' '}
          <MathEquation latex="f_{\text{abs}}" /> を
          用いてシンボリック表現に変換し、学習したオペレータの集合 <MathEquation latex="O" />{' '}
          を用いて 計画を生成します。計画が生成できた場合は、各ステップに対して報酬を計算し、計画{' '}
          <MathEquation latex="P" /> に保存します。
        ...
        <p>
          ...
          エージェントがそのステップを達成したかどうかを判定し、達成した場合に報酬{' '}
          <MathEquation latex="r_i" /> を与えます。
        </p>

        <p>PRMの報酬関数は以下のように定義されます：</p>
      ...
      <SubsectionContainer title="好奇心駆動型想像力" id="curiosity-driven-imagination">
        ...
        <p>
          ...
          各ステップで、まずアクション誘導計画（AGP）を用いて計画 <MathEquation latex="P" />{' '}
          を生成します。 計画が生成できた場合は、PRMからの報酬{' '}
          <MathEquation latex="r_{\text{PRM}}" /> を用いて行動を選択します。
          計画が生成できない場合は、ICMからの報酬 <MathEquation latex="r_{\text{ICM}}" />{' '}
          を用いて新しい遷移の探索を促進します。
        </p>

        <p>
          新しい遷移 <MathEquation latex="(s, a, s')" /> が観測された場合は、 オペレータ抽出関数{' '}
          <MathEquation latex="\text{ExtractOperator}" /> を用いて新しいオペレータ
          <MathEquation latex="o" /> を抽出し、オペレータの集合 <MathEquation latex="O" />{' '}
          に追加します。
        ...
        <p>
          内発的好奇心モジュール（ICM）は、状態 <MathEquation latex="s" /> と行動{' '}
          <MathEquation latex="a" /> から 次の状態 <MathEquation latex="s'" />{' '}
          を予測するモデルとして実装されます。
        ...
        <p>
          ここで <MathEquation latex="\hat{s}'" /> はICMによる次状態の予測、
          <MathEquation latex="\eta" /> はスケーリング係数です。
          この報酬関数により、エージェントは予測困難な状態遷移を探索するよう促されます。
        </p>

        <h4 className="text-lg font-semibold mb-2 mt-4">報酬の統合</h4>
        <p>
          Bi-Modelでは、外部報酬 <MathEquation latex="r_{\text{ext}}" />
          、ICMからの内部報酬 <MathEquation latex="r_{\text{ICM}}" />、 PRMからの計画報酬{' '}
          <MathEquation latex="r_{\text{PRM}}" /> を適切に統合することで、
        ...
        <p>
          ここで <MathEquation latex="\lambda_{\text{PRM}}" /> と{' '}
          <MathEquation latex="\lambda_{\text{ICM}}" /> は、
      ...
      <AITips>
        <p>Bi-Modelの本質をより深く理解するために、日常的な例で考えてみましょう。</p>
        <p>
          <strong>内発的好奇心モジュール（ICM）</strong>
          を、未知の街を探索する旅行者に例えることができます。
        ...
        <p>
          <strong>好奇心駆動型想像力（CDI）</strong>
          は、この2つの旅行スタイルを組み合わせた賢い旅行者です。
