...
export default function Sidebar({ paperData }) {
  ...
  const renderPaperInfo = () => {
    ...
    return (
      <div className="mb-6">
        <p className="text-sm mb-2"><span className="font-semibold">タイトル:</span> {paper.title}</p>
        ...
        <p className="text-sm mb-4"><span className="font-semibold">出版年:</span> {paper.publishedYear}</p>
      </div>
    );
  };
